...
#define ADC_VREF 3.3
#define CURRENT_SENSOR_SENSIBILITY 0.04
#define SIGNAL_FREQUENCY 60
#define MOVING_AVERAGE_SAMPLES 10
#define ACTIVITY_THRESHOLD 0.1
#define VOLTAGE_SENSOR_ALPHA 0.1872
...
TimerHandle_t buttonDebounceTimer;

uint16_t current_offset = 0;
uint16_t voltage_offset = 1877;

const uint32_t period_us = 10e6 / SIGNAL_FREQUENCY;
...
void measure() {
  uint32_t Isum = 0, Vsum = 0, measurements_count = 0;

  uint32_t t_start_us = micros();
  while (micros() - t_start_us < period_us) {
    uint16_t Ireading = analogRead(CURRENT_SENSOR_PIN);
    uint16_t Vreading = analogRead(VOLTAGE_SENSOR_PIN);

    int32_t Inow = Ireading - current_offset;
    Isum += Inow * Inow;

    int16_t Vnow = Vreading - voltage_offset;
    Vsum += Vnow * Vnow;

    measurements_count++;
  }

  float Irms = sqrt(Isum / measurements_count) / ADC_RESOLUTION * ADC_VREF / CURRENT_SENSOR_SENSIBILITY;
  current = Irms;

  float Vrms = sqrt(Vsum / measurements_count) * VOLTAGE_SENSOR_ALPHA;
  voltage = Vrms;

  DEBUG && Serial.print("Measured current: ");
  DEBUG && Serial.println(current);
  DEBUG && Serial.print("Measured voltage: ");
...
void mqttPublishData() {
  ...
  float Ifinal = (!relay_status || current <= ACTIVITY_THRESHOLD) ? 0 : current;
...
void buttonInterrupt() {
  xTimerStart(buttonDebounceTimer, pdMS_TO_TICKS(debounceDelay));
}
...
void setup() {
  ...
  measurementTimer = xTimerCreate("measurementTimer", pdMS_TO_TICKS(MEASURE_INTERVAL_MS), pdTRUE, (void*)0, reinterpret_cast<TimerCallbackFunction_t>(measure));
  buttonDebounceTimer = xTimerCreate("buttonDebounceTimer", pdMS_TO_TICKS(debounceDelay), pdFALSE, (void*)0, reinterpret_cast<TimerCallbackFunction_t>(buttonDebounceCallback));
  ...
  current_offset = getSensorZeroOffset();
  delay(100);
  ...
  xTimerStart(measurementTimer, pdMS_TO_TICKS(500));
}
